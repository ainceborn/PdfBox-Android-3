...
import com.ainceborn.pdfbox.cos.ICOSParser;
...
import com.ainceborn.pdfbox.pdmodel.encryption.DecryptionMaterial;
import com.ainceborn.pdfbox.pdmodel.encryption.InvalidPasswordException;
import com.ainceborn.pdfbox.pdmodel.encryption.PDEncryption;
import com.ainceborn.pdfbox.pdmodel.encryption.ProtectionPolicy;
import com.ainceborn.pdfbox.pdmodel.encryption.PublicKeyDecryptionMaterial;
import com.ainceborn.pdfbox.pdmodel.encryption.SecurityHandler;
import com.ainceborn.pdfbox.pdmodel.encryption.StandardDecryptionMaterial;
...
public class COSParser extends BaseParser implements ICOSParser
{
    ...
    static
    {
        ...
        catch (IllegalArgumentException | UnsupportedOperationException e)
        {
            ...
            Log.w("PdfBox-Android",
                    "Charset is not supported: " + charsetName +
                            ", falling back to " + StandardCharsets.ISO_8859_1.name(),
                    e);
        }
    ...
    private void init(StreamCacheCreateFunction streamCacheCreateFunction)
    {
        ...
        if (eofLookupRangeStr != null)
        {
            ...
            {
                Log.w("PdfBox-Android",
                        "System property " + SYSPROP_EOFLOOKUPRANGE +
        ...
        try
        {
            ...
            while (((i = source.peek()) > 0) && ((char) i != ']'))
            {
                ...
                if (pbo instanceof COSObject)
                {
                    ...
                    if (po.size() > 1 && po.get(po.size() - 1) instanceof COSInteger)
                    {
                        ...
                        if (po.size() > 0 && po.get(po.size() - 1) instanceof COSInteger)
                        {
                            ...
                            {
                                Log.w("PdfBox-Android", "Invalid value(s) for an object key " + number.longValue() + " " + genNumber.intValue());
                            }
                        }
                    }
                }
                ...
                if (pbo == null)
                {
                    ...
                    Log.w("PdfBox-Android",
                            "Corrupt array element at offset " + source.getPosition()
        ...
        try
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    Log.w("PdfBox-Android",
                            "Invalid dictionary, found: '" + c +
                                    "' but expected: '/' at offset " +
                                    source.getPosition());
            ...
            {
                Log.w("PdfBox-Android",
                        "Invalid dictionary, can't find end of dictionary at offset "
                                + source.getPosition());
            }
            return obj;
        }
    ...
    private boolean parseCOSDictionaryNameValuePair(COSDictionary obj) throws IOException
    {
        ...
        {
            Log.w("PdfBox-Android",
                    "Empty COSName at offset " + source.getPosition());
        }
        ...
        {
            Log.w("PdfBox-Android",
                    "Bad dictionary declaration at offset " + source.getPosition());
            return false;
        }
        else if (value instanceof COSInteger && !((COSInteger) value).isValid())
        {
            Log.w("PdfBox-Android",
                    "Skipped out of range number value at offset " + source.getPosition());
        }
        else
        {
        ...
        try
        {
            ...
            switch (c)
            {
                ...
                default:
                    ...
                    {
                        Log.w("PdfBox-Android", "Skipped unexpected dir object = '" + badString + "' at offset " + source.getPosition() + " (start offset: " + startOffset + ")");
    ...
    private boolean validateStreamLength(long streamLength) throws IOException
    {
        ...
        if (streamLength == 0)
        {
            ...
            Log.d("PdfBox-Android", "Suspicious stream length 0, start position: " + originOffset);
            return false;
        }
    ...
    private boolean parseHeader(String headerMarker, String defaultVersion) throws IOException
    {
        ...
        {
            Log.d("PdfBox-Android", "Can't parse the header version. " + exception.getMessage(), exception);
        }
        if (headerVersion < 0)
        {
            if (isLenient)
    ...
    private String decodeBuffer(ByteArrayOutputStream buffer)
    {
        ...
        catch (CharacterCodingException e)
        {
            ...
            Log.d("PdfBox-Android",
                    "Buffer could not be decoded using StandardCharsets.UTF_8 - trying "
                            + ALTERNATIVE_CHARSET.name(),
                    e);
            return IOUtils.toStringCompat(buffer, ALTERNATIVE_CHARSET);
        }
    }
